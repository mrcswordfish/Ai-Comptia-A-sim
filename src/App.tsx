...
const GENERATION_KEY = "comptia_a_generation_ai_v1";
...
type GenerationPersisted = {
  ...
  rawItems: any[]; // AI outputs generated so far
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (p) {
      ...
      // If the previous screen was "generating", return to setup and allow explicit resume.
      setScreen(p.screen === "generating" ? "setup" : (p.screen ?? "setup"));
...
async function runGeneration(g: GenerationPersisted) {
  ...
  setGenProgress({
    done: Math.min(nextIndex, plan.length),
    total: plan.length,
    message: g.lastError ? `Resuming after: ${g.lastError}` : "Starting...",
  });
  ...
  try {
    for (let i = nextIndex; i < plan.length; i += batchSize) {
      const chunk = plan.slice(i, i + batchSize);
      setGenProgress({
        done: i,
        total: plan.length,
        message: `Generating questions ${i + 1}-${Math.min(i + batchSize, plan.length)}...`,
      });
      ...
      const updated: GenerationPersisted = { ...g, rawItems: items, nextIndex, lastError: undefined };
    ...
    const finalSession: ExamSession = { ...createEmptySession(selectedCore, config), sessionId, questions: normalized };
    ...
    setResumeGen(updated);
    setScreen("setup");
    if (e?.name !== "AbortError") alert(`AI generation paused. You can resume from Setup. Details: ${msg}`);
  } finally {
